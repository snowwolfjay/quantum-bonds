              ...
              >
                ...
                <ion-label>
                  ...
                  <p class="location-info">
                    {{ person.location || t("计算页面.人员.位置未知") }}
                  </p>
                </ion-label>
...
const infoInputRef = ref<InstanceType<typeof InfoInputComponent>>();
// 国际化
const { t } = useI18n();
...
// 编辑用的人员数据
const editingPerson = ref<Omit<Person, "id">>({
  name: "",
  location: "",
  weight: null,
  height: null,
...
const initData = async () => {
  try {
    ...
    persons.value = dbPersons.filter(
      (person) => person.id !== defaultPerson.id
    );

    // 监听数据变化，并过滤掉默认人员
    unsubscribeFromPersons = await dbService.watchPersons((newPersons) => {
      persons.value = newPersons.filter(
        (person) => person.id !== defaultPerson.id
      );
    });
  } catch (error) {
